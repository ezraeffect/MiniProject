...
>>         }
>>
>>         // 계산기 초기화 이후 현재 입력 숫자가 "0"이거나, 사칙연산 버튼을 눌렸다면, strInputText에 방금 입력한 값 대입한다.
>>
...
>>         }
>>
>>         // "0" : 기본 상태, "" : 아무 숫자도 없음, 길이가 1 : 숫자 한자리 -> 백스페이스로 지울 필요 없이 "0"으로 유지함
>>
...
>> {
>>    // strCalcOperator 문자열 (현재 입력된 연산자)을 내부 열거형(enum) 연산자로 변환하고 계산기 Class에 설정한다.
...
>> {
>>     // 방금 전에 이퀄(=) 연산자가 입력되었다면 계산이 끝난 상태이므로 해당 Flag만 해제하고 연산작업은 수행하지 않는다. - 부호만 바꾼다.
>>     if (isEqualAssignTriggerdOn == true)
>>     {
>>         isEqualAssignTriggerdOn = false;
>>     }
>>    // 이전 연산자 (lastCalcOperator)가 존재하고 현재 연산자(currentCalcOperator)와 다르다면, 이전 연산자 기준으로 먼저 계산 수행함
...
>>         }
>>     }
>>    // 현재 연산자와 이전 연산자가 같거나, 이전 연산자가 _none 일 경우, 현재 연산자를 기준으로 연산 진행
>>     else
>>    {
